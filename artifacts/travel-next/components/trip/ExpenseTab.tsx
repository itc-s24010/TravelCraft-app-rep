...
export function ExpenseTab({ tripId, categories, onRefresh }: Props) {
  ...
  const [form, setForm] = useState({ categoryId: "", expenseAmount: "", expenseDate: "", paymentMethod: "" });
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      setShowForm(false);
      setForm({ categoryId: "", expenseAmount: "", expenseDate: "", paymentMethod: "" });
      await load();
      onRefresh();
    ...
    <div className="bg-white rounded-xl border border-border p-4">
      ...
      {expenses.length === 0 ? (
        ...
        <div className="divide-y divide-border/50">
          {expenses.map((e) => (
            <div key={e.expenseId} className="py-3 flex items-center gap-3">
