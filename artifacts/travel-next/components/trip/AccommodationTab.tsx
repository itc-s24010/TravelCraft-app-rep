...
export function AccommodationTab({ tripId }: Props) {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      setShowForm(false);
      setForm({ accommodationName: "", address: "", checkIn: "", checkOut: "", reservationNumber: "" });
      await load();
    ...
    <div className="bg-white rounded-xl border border-border p-4">
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="grid grid-cols-2 gap-2 mb-4 p-3 bg-muted/30 rounded-lg">
          <input value={form.accommodationName}
            ...
            className="col-span-2 px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/50"
            placeholder="宿泊施設名" />
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-3">
          {items.map((item) => (
            <div key={item.accommodationId} className="p-3 rounded-lg border border-border/70 bg-muted/20">
