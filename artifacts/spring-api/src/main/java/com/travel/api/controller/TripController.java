...
public class TripController {
    ...
    private User resolveUser(UserPrincipal principal) {
        return userService.ensureUser(principal.getSupabaseUserId());
    }

    @GetMapping
    public List<Trip> getAll(@AuthenticationPrincipal UserPrincipal principal) {
        return tripRepository.findByUserAndDeletedAtIsNullOrderByCreatedAtDesc(resolveUser(principal));
    }
    ...
    public Trip getOne(@AuthenticationPrincipal UserPrincipal principal,
        ...
        return tripRepository.findByTripIdAndUserAndDeletedAtIsNull(tripId, resolveUser(principal))
    ...
    public Trip update(@AuthenticationPrincipal UserPrincipal principal,
                       @PathVariable Long tripId,
                       @RequestBody TripRequest req) {
        Trip trip = tripRepository.findByTripIdAndUserAndDeletedAtIsNull(tripId, resolveUser(principal))
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));
    ...
    public SummaryResponse getSummary(@AuthenticationPrincipal UserPrincipal principal,
                                       @PathVariable Long tripId) {
        Trip trip = tripRepository.findByTripIdAndUserAndDeletedAtIsNull(tripId, resolveUser(principal))
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));
        return summaryService.getSummary(trip);
    }
}
